...
class WavePDE(nn.Module):
    def __init__(self, num_support_sets, num_support_timesteps, support_vectors_dim):
        ...
        self.num_support_sets = num_support_sets  # eventually make continuous?
        self.num_support_timesteps = num_support_timesteps
        self.support_vectors_dim = support_vectors_dim  # latent dim
        self.c = nn.Parameter(torch.ones(num_support_sets,1,requires_grad=True)) # for f

        # N.B. each is a potential, so a functional with 1 output neuron
    ...
    #Loss of PDE
    def loss_pde(self, mlp, z, t, c):
        """loss pde, combines gradient wrt time and latent state 
        """
        z = z.clone().requires_grad_()
        #z.requires_grad = True
    ...
    def forward(self, index, z, t, generator):
        ...
        # Run only half of the traj
        for i in range(0, half_range):
            t_index = i*torch.ones(1, 1, requires_grad=True, zdevice=z.device)
